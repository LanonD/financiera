...
.cl-sub{font-size:13px;color:var(--text-muted)}
.info-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px}
.info-card{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:14px 16px}
.info-card-label{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.07em;color:var(--text-muted);margin-bottom:4px}
...
.score-txt{font-size:9px;font-weight:600;text-transform:uppercase;color:var(--text-muted);margin-top:1px}
.score-card{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:16px 20px;display:flex;align-items:center;gap:20px;margin-bottom:24px}
.score-stats{display:flex;gap:24px;flex-wrap:wrap}
.score-stat{text-align:center}
.score-stat-val{font-size:18px;font-weight:700}
.score-stat-lbl{font-size:10px;font-weight:600;text-transform:uppercase;color:var(--text-muted)}
.loan-block{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);margin-bottom:16px;overflow:hidden}
...
.loan-block-header:hover{background:var(--bg-hover)}
.loan-title{font-size:13px;font-weight:600;flex:1}
.loan-meta{font-size:12px;color:var(--text-muted);display:flex;gap:16px}
.loan-chevron{transition:transform .2s;color:var(--text-muted)}
...
.pago-row-pendiente{background:var(--bg-hover);opacity:.7}
...
.diff-badge{font-size:11px;font-weight:700;font-family:var(--font-mono)}
.back-link{display:inline-flex;align-items:center;gap:6px;font-size:12px;color:var(--text-muted);margin-bottom:16px;cursor:pointer;text-decoration:none}
...
<div class="cl-header">
    ...
    <div>
        <div class="cl-name"><?= htmlspecialchars($cliente['nombre']) ?></div>
        <div class="cl-sub"><?= htmlspecialchars($cliente['ocupacion']??'—') ?> · <?= htmlspecialchars($cliente['celular']??'—') ?></div>
    </div>
</div>
...
<div class="info-grid">
    <div class="info-card">
        ...
        <div class="info-card-value" style="font-size:13px"><?= htmlspecialchars($cliente['direccion']??'—') ?></div>
    </div>
    <div class="info-card">
        <div class="info-card-label">CURP</div>
        <div class="info-card-value" style="font-size:12px;font-family:var(--font-mono)"><?= $cliente['curp']??'—' ?></div>
    </div>
    <div class="info-card">
        <div class="info-card-label">Promotor</div>
        <div class="info-card-value"><?= htmlspecialchars($cliente['promotor_nombre']??'—') ?></div>
    </div>
    <div class="info-card">
        <div class="info-card-label">Préstamos</div>
        <div class="info-card-value"><?= count($prestamos) ?></div>
    </div>
</div>
...
<div class="score-card">
    <div class="score-ring">
        <?php $pct = $score ?? 0; $circ = 2 * M_PI * 30; $dash = $circ * $pct / 100; ?>
...
<div class="table-card" style="text-align:center;padding:40px;color:var(--text-muted)">Este cliente no tiene préstamos registrados</div>
<?php endif; ?>

<?php foreach ($prestamos as $idx => $loan):
    $pagosDelPrestamo = $loan['pagos'];
    $totalPagadoMonto = array_sum(array_column(array_filter($pagosDelPrestamo, fn($p) => in_array($p['estatus'],['Pagado','Parcial'])), 'monto_cobrado'));
    $pagosCount = count(array_filter($pagosDelPrestamo, fn($p) => in_array($p['estatus'],['Pagado','Parcial'])));
    $pendCount  = count(array_filter($pagosDelPrestamo, fn($p) => in_array($p['estatus'],['Pendiente','Atrasado'])));
?>
<div class="loan-block">
    <div class="loan-block-header" onclick="toggleLoan(<?= $loan['id'] ?>)">
        <div style="display:flex;flex-direction:column;gap:3px;flex:1">
            <div class="loan-title">
                ...
                <span class="badge badge-<?= strtolower($loan['estatus']) ?>" style="margin-left:8px;font-size:11px"><span class="dot"></span><?= $loan['estatus'] ?></span>
            </div>
            <div class="loan-meta">
                ...
                <span>Promotor: <?= htmlspecialchars($loan['promotor_nombre']??'—') ?></span>
            </div>
        </div>
    ...
    <div class="loan-body <?= $idx === 0 ? 'open' : '' ?>" id="body-<?= $loan['id'] ?>">
        <?php if (empty($pagosDelPrestamo)): ?>
        <p style="padding:20px;color:var(--text-muted);font-size:13px">Sin historial de pagos registrado.</p>
        <?php else: ?>
        ...
        <table style="width:100%">
            ...
            <tbody>
            <?php foreach ($pagosDelPrestamo as $p):
                $paid    = in_array($p['estatus'], ['Pagado', 'Parcial']);
                $diff    = $paid ? (int)$p['dias_diff'] : null;
                $rowCls  = 'pago-row-pendiente';
                $diffTxt = '—';
                $diffClr = 'var(--text-muted)';
                ...
                $statusPill = match($p['estatus']) {
                    'Pagado'   => ['#dcfce7','#166534','Pagado'],
                    'Parcial'  => ['#fef9c3','#854d0e','Parcial'],
                    'Atrasado' => ['#fee2e2','#991b1b','Atrasado'],
                    default    => ['var(--bg-hover)','var(--text-muted)','Pendiente'],
                };
            ?>
            <tr class="<?= $rowCls ?>">
                <td style="text-align:center;font-weight:600;font-size:12px"><?= $p['numero_pago'] ?></td>
                <td class="td-numeric"><?= date('d/m/Y', strtotime($p['fecha_programada'])) ?></td>
                <td class="td-numeric"><?= $p['fecha_pago'] ? date('d/m/Y', strtotime($p['fecha_pago'])) : '—' ?></td>
                <td><span class="diff-badge" style="color:<?= $diffClr ?>"><?= $diffTxt ?></span></td>
                <td class="td-amount">$<?= number_format($p['monto_cuota'],2,'.',',') ?></td>
                <td class="td-amount"><?= $p['monto_cobrado'] ? '$'.number_format($p['monto_cobrado'],2,'.',',') : '—' ?></td>
                <td class="td-amount">$<?= number_format($p['capital'],2,'.',',') ?></td>
